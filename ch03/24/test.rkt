...
;; 3.23 updated to the curried version for simplification
(test-case
    "solution to 3.23"
  (check-equal? (value-of-program (scan&parse "let makemult = proc (maker)
                                               ...
                                               in let times4 = proc (x) ((makemult makemult) x)
                                                  in (times4 3) "))
                (num-val 12))
  (check-equal? (value-of-program (scan&parse "let makefact = proc (maker x)
                                                                if zero?(x)
                                                                  then 1
...
(test-case
  ...
  (check-equal? (value-of-program
                 (scan&parse
                  "let makeeven = proc (even odd x)
                                       if zero?(x)
                                       then 1
                                       else (odd even odd -(x, 1))
                   in let makeodd = proc (even odd x)
                                        if zero?(x)
                                        then 0
                                        else (even even odd -(x, 1))
                      in let even = proc (x) (makeeven makeeven makeodd x)
                         in let odd = proc (x) (makeodd makeeven makeodd x)
                            in cons((even 10),cons((odd 10), emptylist))"))
                (cons-val (num-val 1)
                          (cons-val (num-val 0)
                                    (emptylist-val)))))
