...
(test-case "check list functions right"
  (check-equal? (run "null?(list())") (bool-val #t))
  (check-equal? (run "car(list(1,2,3))") (num-val 1))
  (check-equal? (run "cdr(cons(3, emptylist))") (emptylist-val)))
...
(test-case
    "exercise 4.7"
  (check-equal? (run "let x = newref(0) in setref(x, 1)")
                (num-val 0)))
